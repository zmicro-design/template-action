...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.Fz = void 0;
const add_1 = __nccwpck_require__(2908);
const qs = __nccwpck_require__(6854);
const lru_1 = __nccwpck_require__(2790);
const url_join_1 = __nccwpck_require__(2122);
const utils_1 = __nccwpck_require__(7231);
class Fz {
    constructor(config) {
        this.config = config;
        this.requestConfig = {};
        this.engine = config.engine || Fz._ENGINE || utils_1.fetch;
    ...
    static enableShowLoading() {
        Fz._DEFAULT_SHOW_LOADING = true;
    }
    static beforeRequest(handler) {
        Fz._HOOKS.beforeRequest.push(handler);
    }
    static afterResponse(handler) {
        Fz._HOOKS.afterResponse.push(handler);
    }
    static onBadRequest(handler) {
        Fz.status(400, handler);
    }
    static onUnauthorized(handler) {
        Fz.status(401, handler);
    }
    static onForbidden(handler) {
        Fz.status(403, handler);
    }
    static onNotFound(handler) {
        Fz.status(404, handler);
    }
    static onMethodNotAllowed(handler) {
        Fz.status(405, handler);
    }
    static onRateLimited(handler) {
        Fz.status(429, handler);
    }
    static onInternalServerError(handler) {
        Fz.status(500, handler);
    }
    static onBadGateway(handler) {
        Fz.status(502, handler);
    }
    static onServiceUnavailable(handler) {
        Fz.status(503, handler);
    }
    static onGatewayTimeout(handler) {
        Fz.status(504, handler);
    }
    static engine(customEngine) {
    ...
    applyQuery() {
        ...
        if (query && Object.keys(query).length > 0) {
            ...
            if (index !== -1) {
                this.requestConfig.url = `${this.requestConfig.url.slice(0, index)}?${(0, add_1.add)(this.requestConfig.url.slice(index), query)}`;
            }
            else {
    ...
    applyBaseUrl() {
        if (Fz._BASE_URL) {
            this.requestConfig.url = (0, url_join_1.urlJoin)(Fz._BASE_URL, this.requestConfig.url);
        }
    }
    applyHeader() {
        const headers = this.requestConfig.headers = new utils_1.Headers(Fz._HEADERS);
        const optionHeaders = this.config.headers;
        for (const key in optionHeaders) {
            headers.set(key, optionHeaders[key]);
        }
        if (!headers.get('content-type') && (0, utils_1.isPlainObject)(this.config.body)) {
    ...
    applyCache() {
        if (this.config.cache) {
            Fz._CACHE = Fz._CACHE || new lru_1.default();
        }
    }
    applyStatus() {
        const _sh = Fz._STATUS;
        const af = async (response, options) => {
    ...
    async request(finalConfig) {
        return await (0, utils_1.timeout)(this.engine.call(this.engine, this.requestConfig.url, finalConfig), this.timeout);
    }
    async response() {
    ...
    async arrayBuffer() {
        return this.getResponse("arrayBuffer");
    }
    async blob() {
        return this.getResponse("blob");
    }
    async getResponse(type) {
        await this.beforeRequest(this.requestConfig);
    ...
    async retry(fn) {
        return (0, utils_1.retry)(fn, this.retryCount, (error) => {
            if (error instanceof utils_1.HTTPError || error instanceof utils_1.TimeoutError) {
                return true;
            }
            return false;
        });
    ...
    async setCachedResponse(options, response) {
        var _a;
        if (!this.config.cache) {
            return;
        }
        ...
        return Fz._CACHE.set(key, response, {
            maxAge: (_a = this.config.cache) === null || _a === void 0 ? void 0 : _a.maxAge,
        });
    }
    async getCachedKey(options) {
        return JSON.stringify(options);
    }
    async runStatusBeforeError(response) {
        const _sh = Fz._STATUS;
        const af = async (response, options) => {
            const status = response.status;
            const handlers = _sh[status] || [];
...
Fz._STATUS = {};
Fz._LOADING = {};
Fz._DEFAULT_SHOW_LOADING = false;
Fz._HOOKS = {
    beforeRequest: [],
    afterResponse: [],
};
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.fz = void 0;
__exportStar(__nccwpck_require__(5459), exports);
__exportStar(__nccwpck_require__(7231), exports);
const core_1 = __nccwpck_require__(3320);
exports.fz = core_1.Fz;
exports["default"] = core_1.Fz;
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.HTTPError = exports.TimeoutError = void 0;
...
class HTTPError extends Error {
    constructor(status, codeMessage, response, request) {
...
const ismorphicFetch = __nccwpck_require__(2340);
const hasFetch = () => typeof window !== 'undefined' && !!window.fetch;
exports.fetch = hasFetch() ? window.fetch.bind(window) : ismorphicFetch;
...
const userAgent_1 = __nccwpck_require__(7236);
class Headers {
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.isPlainObject = exports.retry = exports.timeout = exports.fetch = exports.Headers = void 0;
__exportStar(__nccwpck_require__(6614), exports);
var headers_1 = __nccwpck_require__(473);
Object.defineProperty(exports, "Headers", ({ enumerable: true, get: function () { return headers_1.Headers; } }));
var fetch_1 = __nccwpck_require__(1180);
Object.defineProperty(exports, "fetch", ({ enumerable: true, get: function () { return fetch_1.fetch; } }));
var timeout_1 = __nccwpck_require__(1431);
Object.defineProperty(exports, "timeout", ({ enumerable: true, get: function () { return timeout_1.timeout; } }));
var retry_1 = __nccwpck_require__(7064);
Object.defineProperty(exports, "retry", ({ enumerable: true, get: function () { return retry_1.retry; } }));
var isPlainObject_1 = __nccwpck_require__(8199);
...
const delay_1 = __nccwpck_require__(7639);
const retry = (fn, times = 0, isKnownError) => {
    let runtimes = 0;
    const next = async () => {
        try {
            return await fn();
...
const error_1 = __nccwpck_require__(6614);
const timeout = async (promise, ms, context) => {
    let it;
    const timeoutPromise = new Promise((resolve, reject) => {
        it = setTimeout(() => reject(new error_1.TimeoutError(context === null || context === void 0 ? void 0 : context.request)), ms);
    });
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
const pkg = __nccwpck_require__(9208);
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
var parse_1 = __nccwpck_require__(7025);
var stringify_1 = __nccwpck_require__(2354);
exports.add = function (queryString, query) {
    if (query === void 0) { query = {}; }
    var oqs = parse_1.parse(queryString);
    var nqs = Object.assign(oqs, query);
    return stringify_1.stringify(nqs);
};
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
__export(__nccwpck_require__(7025));
__export(__nccwpck_require__(2354));
...
exports.parse = function (query) {
    ...
    return realQuery
        ...
        var _a = item.split('='), key = _a[0], vs = _a.slice(1);
        var value = decodeURIComponent(vs.join('='));
        if (!total[key]) {
            total[key] = value;
        }
...
const psTree = __nccwpck_require__(2757);
const exit_codes_1 = __nccwpck_require__(6246);
const event_1 = __nccwpck_require__(9795);
const execa_1 = __nccwpck_require__(2862);
const utils_1 = __nccwpck_require__(9369);
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.execa = exports.$ = exports.registerGlobals = void 0;
const which = __nccwpck_require__(4207);
const utils_1 = __nccwpck_require__(9369);
const commands_1 = __nccwpck_require__(98);
function registerGlobals() {
    const os = __nccwpck_require__(2037);
    const fs = __nccwpck_require__(7147);
    const path = __nccwpck_require__(1017);
    const fetch = __nccwpck_require__(467);
    const fz = (__nccwpck_require__(3591).fz);
    const globbyModule = __nccwpck_require__(3398);
    const sleep = (ms) => new Promise((resolve) => setTimeout(resolve, ms));
    // @example
    //  const packages = await globby(['package.json', 'packages/*/package.json']);
...
function $(pieces, ...args) {
    var _a;
    const getCommand = utils_1.createGetCommand({ strict: $.strict });
    const __from = (_a = new Error().stack) === null || _a === void 0 ? void 0 : _a.split('at ')[2].trim();
    const cmd = getCommand(pieces, ...args);
    if ($.verbose) {
        console.log('$', utils_1.colorize(cmd));
    }
    const options = {
        windowsHide: true,
        // stdio: ['inherit', 'pipe', 'pipe'],
    ...
    if (__from)
        options.__from = __from;
    return commands_1.exec(cmd, options);
}
exports.$ = $;
...
$.raw = undefined;
// for safe, default is strict
// if true, not allow $`${cmd}`
//    only allow $`git commit -m "${message}"`
$.strict = true;
// declaretive unsafe command, which is good usage
// @example
//  $.unsafe`${cmd}`
$.unsafe = commands_1.unsafe;
//
$.cd = commands_1.cd;
//
$.kill = commands_1.kill;
//
$.spawn = commands_1.spawn;
//
$.runShell = commands_1.runShell;
exports.execa = $;
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
const execa_1 = __nccwpck_require__(2862);
__exportStar(__nccwpck_require__(2862), exports);
exports["default"] = execa_1.execa;
...
const chalk = __nccwpck_require__(9220);
// function d(m: string) {
//   return m.length === 1 ? "'\\''" : `'"${m}"'`;
// }
// const n = /\x00+/g;
...
const chalkTag = (chalk, ...strings) => {
	...
	if (template === undefined) {
		template = __nccwpck_require__(361);
	}
...
const codes = {
    ...
    134: 'Process aborted',
    135: 'Bus error: "access to unedfined portion of memory object"',
    136: 'Floating point exception: "erroneous arithmetic operation"',
    137: 'Kill (terminate immediately)',
    138: 'User-defined 1',
    139: 'Segmentatiion violation',
    140: 'User-defined 2',
    141: 'Write to pipe with no one reading',
    142: 'Signal raised by alarm',
    143: 'Termination (request to terminate)',
    145: 'Child process terminated, stopped (or continuted*)',
    146: 'Continue if stopped',
    147: 'Stop executing temporarily',
    148: 'Terminal stop signal',
    149: 'Bakcground process attempting to read from tty ("in")',
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
const codes_1 = __nccwpck_require__(7557);
__exportStar(__nccwpck_require__(7557), exports);
exports["default"] = codes_1.get;
...
const once_1 = __nccwpck_require__(6357);
...
const queueMicrotask = __nccwpck_require__(1483)
...
"use strict";
module.exports = JSON.parse('{"name":"@zcorky/fz","version":"1.2.15","description":"A simply fetch library","main":"lib/index.js","repository":"https://github.com/zcorky/fz","author":"Zero","license":"MIT","scripts":{"dev":"tsc -w","build":"npm run clean && tsc","clean":"rimraf -rf lib","test":"NODE_ENV=test jest -w 1 --detectOpenHandles","coverage":"codecov","prepublish":"npm run build"},"devDependencies":{"@types/isomorphic-fetch":"^0.0.35","@types/jest":"^27.0.1","@types/node":"^16.9.2","codecov":"^3.8.3","create-test-server":"^2.4.0","jest":"^27.2.0","mocha":"^5.2.0","node-fetch":"2.6.4","rimraf":"^2.6.2","source-map-support":"^0.5.9","ts-jest":"^27.0.5","ts-node":"^10.2.1","typescript":"^4.4.3"},"files":["lib"],"jest":{"roots":["test"],"transform":{"^.+\\\\.tsx?$":"ts-jest"},"testRegex":"test/.*\\\\.spec\\\\.(ts|tsx)$","moduleDirectories":["node_modules","src"],"moduleFileExtensions":["ts","tsx","js","jsx"],"coverageDirectory":"./coverage/","collectCoverage":true,"collectCoverageFrom":["src/**/*.{ts,tsx}"],"coverageThreshold":{"global":{"branches":70,"functions":60,"lines":80,"statements":-60}},"testTimeout":10000},"publishConfig":{"access":"public"},"dependencies":{"@zcorky/delay":"^1.0.1","@zcorky/lru":"^1.0.5","@zcorky/query-string":"^1.0.2","@zodash/url-join":"^0.1.6","isomorphic-fetch":"^3.0.0"}}');
...
const execa_1 = __nccwpck_require__(485);
